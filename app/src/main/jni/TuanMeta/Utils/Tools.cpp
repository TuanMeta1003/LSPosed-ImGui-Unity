...
uintptr_t Tools::FindPattern(const char *lib, const char *pattern) {
	...
	for (uintptr_t pCur = start; pCur < end; ++pCur) {
		if (*(uint8_t *) curPat == (uint8_t) '\?' || *(uint8_t *) pCur == getByte(curPat)) {
			...
			curPat += (*(uint16_t *) curPat == 0x3F3F || *(uint8_t *) curPat != 0x3F) ? 2 : 1;
			if (!*curPat) {
				return firstMatch;
			}
			curPat++;
...
const char *Tools::GetAndroidID(JNIEnv *env, jobject context) {
	...
	jmethodID getContentResolverMethod = env->GetMethodID(contextClass, oxorany("getContentResolver"), oxorany("()Landroid/content/ContentResolver;"));
	jclass settingSecureClass = env->FindClass(oxorany("android/provider/Settings$Secure"));
	jmethodID getStringMethod = env->GetStaticMethodID(settingSecureClass, oxorany("getString"), oxorany("(Landroid/content/ContentResolver;Ljava/lang/String;)Ljava/lang/String;"));

	auto obj = env->CallObjectMethod(context, getContentResolverMethod);
	auto str = (jstring) env->CallStaticObjectMethod(settingSecureClass, getStringMethod, obj, env->NewStringUTF(oxorany("android_id")));
	return env->GetStringUTFChars(str, 0);
}

const char *Tools::GetDeviceModel(JNIEnv *env) {
	jclass buildClass = env->FindClass(oxorany("android/os/Build"));
	jfieldID modelId = env->GetStaticFieldID(buildClass, oxorany("MODEL"), oxorany("Ljava/lang/String;"));
...
const char *Tools::GetDeviceBrand(JNIEnv *env) {
	jclass buildClass = env->FindClass(oxorany("android/os/Build"));
	jfieldID modelId = env->GetStaticFieldID(buildClass, oxorany("BRAND"), oxorany("Ljava/lang/String;"));
...
const char *Tools::GetDeviceUniqueIdentifier(JNIEnv *env, const char *uuid) {
	jclass uuidClass = env->FindClass(oxorany("java/util/UUID"));
	...
	jmethodID nameUUIDFromBytesMethod = env->GetStaticMethodID(uuidClass, oxorany("nameUUIDFromBytes"), oxorany("([B)Ljava/util/UUID;"));
	jmethodID toStringMethod = env->GetMethodID(uuidClass, oxorany("toString"), oxorany("()Ljava/lang/String;"));
