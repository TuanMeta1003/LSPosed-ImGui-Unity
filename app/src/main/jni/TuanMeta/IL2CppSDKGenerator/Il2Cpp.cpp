...
std::string utf16_to_utf8(const UTF16* source, size_t len) {
    std::u16string s(reinterpret_cast<const char16_t*>(source), len);
    std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t> convert;
    return convert.to_bytes(s);
}

std::wstring utf16_to_utf32(const UTF16* source, size_t len) {
    std::wstring output;
    output.reserve(len);  // Dự phòng dung lượng
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        else if (i + 1 < len && is_high_surrogate(uc) && is_low_surrogate(source[i + 1])) {
            output.push_back(surrogate_to_utf32(uc, source[i + 1]));
            ++i;  // Bỏ qua ký tự low surrogate đã xử lý
        }
        else {
            output.push_back(L'?');
        }
...
std::string Il2CppString::CString() {
    return utf16_to_utf8(&this->start_char, this->length);
}

std::wstring Il2CppString::WCString() {
...
void Il2Cpp::GetStaticFieldValue(const char *image, const char *namespaze, const char *clazz, const char *name, void *output) {
	...
	if(!klass) {
		LOGE("Can't find class %s for field %s!", clazz, name);
		return;
	}

	void *field = il2cpp_class_get_field_from_name(klass, name);
	if(!field) {
		LOGE("Can't find field %s in class %s!", name, clazz);
		return;
	}
...
void *Il2Cpp::GetClass(const char *image, const char *namespaze, const char *clazz) {
	...
	if(classes.size() > 1)
	{
		...
		LOGE("Can't find subclass %s in class %s!", classes[1].c_str(), classes[0].c_str());
		return 0;
	}
...
void *Il2Cpp::NewClassObject(const char *image, const char *namespaze, const char *clazz) {
	...
	if(!obj)
	{
		LOGE("Can't create object for %s", clazz);
		return 0;
	}
...
void *Il2Cpp::GetMethodOffset(const char *image, const char *namespaze, const char *clazz, const char *name, int argsCount) {
	...
	void *klass = GetClass(image, namespaze, clazz);
	if(!klass) {
		LOGE("Can't find class %s for method %s!", clazz, name);
		return 0;
	}

	void **method = (void**)il2cpp_class_get_method_from_name(klass, name, argsCount);
	if(!method) {
		LOGE("Can't find method %s in class %s!", name, clazz);
		return 0;
	}
			...
			for (int i = 0; i < argsCount; i++) {
				...
				if (arg) {
					...
					if (strcmp(tname, args[i]) == 0) {
						...
						LOGE("Argument at index %d didn't matched requested argument!\r\n\tRequested: %s\r\n\tActual: %s\r\nSkipping function...", i, args[i], tname);
						score = 0;
goto skip;
					}
				}
	...
	LOGE("Cannot find function %s in class %s!", name, clazz);
	return 0;
}
